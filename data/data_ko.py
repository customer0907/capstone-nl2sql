...
def parse_sql(sql, max_num, table, question_toks, tokenizer, all_labels, sql_labels, count, spc=["none"]) :
    ...
    for sql_ in sqls :
        if sql_ is not None : # sqls = [None, None] -> 실제 데이터 값 들어있음
            parse_sql(sql_, max_num, table, question_toks, tokenizer, all_labels, sql_labels, count, spc+["where"]) # 재귀 함수
            # break
    ...
    labels += (sel_con1_label, ) # 240613 tables.json에서 select 다음 나오는 column
...
def get_table_pkl(config, tokenizer, data):
    ...
    pkl_path = os.path.join(config.pkl_path, config.pkl_name) # /모델소스코드/pkl
    ...
    if os.path.exists(pkl_path) : # 240530 수정 / not os.path -> os.path | pkl_path가 맨 초기 상태로만 되어 있음. tables.json을 수정하면 이 if문을 통과하도록 해야 갱신됨.
        tables = dict()

        print("making pkl -", pkl_path)
        for table in tqdm(data) : # tables.json
...
def _load_samples(tokenizer, filename, tables, table_names, max_num, count):
    ...
    # aa = 0
    for utt_id, sample in enumerate(data): ## dev.json
        # process each sample
        # aa += 1
        # process question
        ...
        samples.append({
            ...
            , "db_id": sample["db_id"]
            , "utt_id": utt_id
            , "question_toks": question # 240617 추가
            , "hardness": sample["hardness"] if "hardness" in sample else evaluator.eval_hardness(sql)
            , "query": sample["query"]
            , "val_1": sample["sql"]["where"][0][3] # 240516 추가 -> 디버깅 결과 보고 having 따로 처리해줘야 하는지 판단
            , "val_2": sample["sql"]["where"][2][3] # 240516 추가
            , "val_3": sample["sql"]["having"][0][3] # 240521 추가
            # 되는지 확인해보고, 잘 되면 having도 val_3, val_4로 추가, limit도 추가
        })
        # a = max(a, len(tokens))
        # c = max(schema_token_len, c)
    
    print("table_map : ", samples["table_map"]) # 240613 추가
...
def load_spider(config, tokenizer) :
    ...
    dev_tables = set()
    # dev_samples = list()
    with open(config.table_path, "r") as handle : # 240507 table_path = data/nia/tables.json | config.json에 있음
        data = json.load(handle)
    ...
    tables = get_table_pkl(config, tokenizer, data) # table // main_ko.py에서 load_spider로 dataset 불러올 때 사용. 
    total_exceed_list = []
    dataset = {}
