...
tokenizer = transformers.AutoTokenizer.from_pretrained('monologg/koelectra-base-v3-discriminator') # 240617 추가
model = transformers.AutoModel.from_pretrained('monologg/koelectra-base-v3-discriminator') # 240617 추가
...
def second_argmax(array): # 240617 추가
    # Find the index of the maximum value
    first_max_index = np.argmax(array)
    # Copy the array and set the maximum value to a very small number
    array_copy = np.copy(array)
    array_copy[first_max_index] = -np.inf
    # Find the index of the second largest value
    second_max_index = np.argmax(array_copy)
    return second_max_index

def get_embedding(text): # 240617 추가
...
def cosine_similarity(vec1, vec2): # 240617 추가
    return np.dot(vec1, vec2) / (np.linalg.norm(vec1) * np.linalg.norm(vec2))


def check_pred(batch_size, preds, table_map, tables, utt_ids, question_toks, _val_1, _val_2, _val_3, _index) : # 240518 _index 추가 | 240617 question_toks 추가
    # 240516 _val_1, _val_2 추가
    # index = _index
    results = dict()
    sqls = [
        {
    ...
    if "select" in preds :
        ...
        for i in range(batch_size) :
            ...
            # for j in range(sel_num[i]+1) :
                ...
                p, l = pred[5] # column , 만약 column이 10개 있는 table이라면, 10개 각각에 해당하는 확률이 분포되어 있을 것.
                p = np.argmax(p[i][j], axis=-1) # DUTYADDR
                p_snd = second_argmax(p[i][j]) # 두번째로 큰 확률을 가지는 column / DUTYNAME / 240617 추가
                
                for tok in question_toks[i]: # 240617 추가
                    if tok == "주소":
                        address_embedding = get_embedding(tok)
                        if cosine_similarity(column_dict[p], address_embedding) > cosine_similarity(column_dict[p_snd], address_embedding):  # 240617 추가
                            p = p  # keep the original highest probability if it matches better with "주소"
                        else:
                            p = p_snd  # use the second highest if it matches better with "주소"
    ...
    def eval_clause(clause_type, preds, results, table_map, used_tables, sqls, where_clause=False) :
        ...
        # preds[where] 의 결과값들을 각각의 변수에 저장
        ...
        for i in range(batch_size) :
            ...
            for conj_pred_index in conj_pred_batch:
                pred_and_or_list.append(Data.CONJ_MAP_REV[conj_pred_index]) # and, or prediction 추가
            ...
            for j in range(range_pred_cond_num) :
                ...
                where_having_unit[1] = pred_op # pred에는 실제 숫자가 들어가 있음
                # 따라서, cond에 직접 접근하는 것이 아니라, np.argmax 함수를 거쳐야 실제 값을 가져올 수 있는 듯 (예상)

                ### 240516 수정
                # where_having_unit[3,4] = val_1, val_2 prediction
    ...
    results["final_sample"] = sample_pred ## 240516 디버깅해보기
    decode_sql_list = decode_sql(sqls, tables, utt_ids, _val_1, _val_2, _val_3, _index) # 240516 val_1, val_2 추가 | 240518 index 추가 | 240521 val_3 추가

    return results, decode_sql_list


def decode_sql(sqls, tables, utt_ids, _val_1, _val_2, _val_3, index): # 240516 추가
    def decode_col_units(col_units, clause="select"):
        ...
        for _j in range(col_num):
            ...
            if clause == "select":
                col_agg = col_unit[0]
                unit_op, cond_unit_1, cond_unit_2 = col_unit[1] # 디버깅
            else:
                col_agg = 0
    ...
    def decode_where_cond_units(target_sql, _val_1, _val_2, _val_3, index, where_clause=True): # 240516 추가
        decode_str = decode_where_cond_unit(target_sql["cond_units"][0],_val_1,_val_2,_val_3, index) # 240516 추가 | # 240521 추가

        if target_sql["and_or"]:
            for _cond_unit, _and_or in zip(target_sql["cond_units"][1:], target_sql["and_or"]):
                decode_str += _and_or.upper()
                decode_str += decode_where_cond_unit(_cond_unit,_val_1,_val_2,_val_3, index, first_call=False, where_clause=where_clause) # 240516 추가 | 240521 추가

        if where_clause == False:
            decode_str = decode_where_cond_unit(target_sql["cond_units"][0],_val_1,_val_2,_val_3, index, where_clause=False, first_call=False) # 240521 추가

        return decode_str

    def decode_where_cond_unit(_cond_unit, _val_1,_val_2, _val_3, index, where_clause=True, first_call=True): # 240516 추가 | 240521 추가
        """
            Use : where condition / having condition
        """

        not_op, op, cond_units, nest_1, nest_2 = _cond_unit
        ### 혹시 cond_units 에 val_1, val_2의 predict값이 들어있나 확인 -> X 없음
        ### test | print("cond_units : ",cond_units)
        ### cond_units :  [0, [0, 3, False], []] 와 같은 form
        not_op_str = "NOT " if not_op else ""

        val_1 = f" {_val_1[index]} " if index < len(_val_1) and _val_1[index] else " NULL "
        val_2 = f" {_val_2[index]} " if index < len(_val_2) and _val_2[index] else " NULL "
        val_3 = f" {_val_3[index]} " if index < len(_val_3) and _val_3[index] else " NULL "
        ...
        # if op:
        #     where_cond_unit = (
        #         decode_col_units([cond_units], clause="where")
        #         + not_op_str
        #         + Data.OPS[op].upper()
        #         + val_1
        #     )
        ...
        # else:
        #     where_cond_unit = f" {DECODE_ERROR_STR} "

        if first_call and op:
            where_cond_unit = decode_col_units([cond_units], clause="where") + not_op_str + Data.OPS[op].upper() + val_1
            if op == Data.OPS.index("between"):
                where_cond_unit += "AND" + val_2
        elif not first_call and val_2.strip() != "NULL" :
            where_cond_unit += decode_col_units([cond_units], clause="where") + not_op_str + Data.OPS[op].upper() + val_2
        else :
            where_cond_unit = f" {DECODE_ERROR_STR} " 

        if where_clause == False:
            where_cond_unit = decode_col_units([cond_units], clause="having") + not_op_str + Data.OPS[op].upper() + val_3
    ...
    for i in range(len(sqls)):
        print("decode_where_cond_unit i : ", i)
        ...
        if limit_sql:
            ...
            if is_top1:
                ...
                """
                decode_result["limit"] = 'LIMIT ' + VALUE_STR # 240519 수정해야함
        ...
        if where_sql["cond_units"]:
            decode_result["where"] = "WHERE" + decode_where_cond_units(where_sql, _val_1, _val_2, _val_3, index) # 240516 val_1, val_2 추가
            # print(utt_ids[i])
            # print(where_sql)
        ...
        if having_sql["cond_units"]:
            decode_result["having"] = "HAVING" + decode_where_cond_units(having_sql, _val_1, _val_2, _val_3, index, where_clause=False) # 240516 val_1, val_2 추가
            # print(utt_ids[i])
            # print(having_sql)
