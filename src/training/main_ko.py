...
import matplotlib.pyplot as plt # 240523 추가
...
def convert_to_tensor(elements, device="cuda") :
    arr = []
    for e in elements :
        arr.append(e)
...
def make_inputs(config, samples, _i, device="cuda") : # 240518 _i 추가
    ...
    for sample_ in samples :
        ...
        val_1.append(sample_["val_1"]) # 240516 추가
        val_2.append(sample_["val_2"]) # 240516 추가
        val_3.append(sample_["val_3"]) # 240521 추가
    ...
    return (x, mask, q_len, c_len, t_len, sql_mask, i, all_labels, table_map, val_1, val_2, val_3) # 240516 val_1, val_2 추가 | 240521 val_3 추가
...
def run_epoch(config, epoch_num, model, optimizer, samples, loader, tables=None, train=True, do_show=False,
    ...
    for i, batch in enumerate(loader) :
        ...
        inputs = make_inputs(config, [samples[_i] for _i in idx], i, config.device) ## 240518 i 추가
        inputs += (train, )
        if tables:
        ...
        if not train:
            utt_ids = [samples[_i]["utt_id"] for _i in idx] # 질문 종류
            question_toks = [samples[_i]["question_toks"] for _i in idx] # 240617 추가 , 질문 토큰
            inputs += (utt_ids, )
            inputs += (question_toks,) # 240617 추가

        loss, result, decode_result = model(*inputs) # decode_result는 sql문을 저장하는 변수 
        ...
        if train :
            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_no:rm_(model.parameters(), 1)
            optimizer.step()
...
def main(config) :
    ...
    dataset, tables, train_tables, dev_tables = Data.load_spider(config, tokenizer) # 그냥 tables.
    # 240516 test
    # dataset, tables, dev_tables 안에 담긴 내용 디버깅
    # dataset : datset["dev"] : samples
    # samples.append({
    #         "tokens": tokens
    #         , "q_len": q_len
    #         , "t_len": t_len
    ...
    model = Model.Model(config) # model_ko 라이브러리 Model 클래스의 객체 생성
    ...
    if config.train:
        ...
        for e in range(config.epoch):
            model.train()
            train_loss, _, _ = run_epoch(config, e, model, optimizer, dataset["train"], train_loader)
            train_losses.append(train_loss)

            model.eval()
            with torch.no_grad():
                valid_loss, final_score, _ = run_epoch(config, e, model, optimizer, dataset["valid"], valid_loader, train=False)
        ...
        plt.savefig(os.path.join(config.save_ckpt, 'loss_plot.png'))
        print(f"Training and validation loss plot saved to {os.path.join(config.save_ckpt, 'loss_plot.png')}")
