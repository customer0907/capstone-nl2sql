...
class Embedding(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        if type(out) == tuple:
            out = out[0]
        return out.to(x.device) # 240524 수정 | out -> out.to(x.device)
...
class TableClause(nn.Module):
    def __init__(self, config):
        ...
        self.dense4 = nn.Linear(config.hidden_size, config.hidden_size) # 240603 추가 | hidden layer 추가
    ...
    def forward(self, v_T, v_Q, v_D, v_P, d_mask, labels=None):
        ...
        table_num_logit = self.dense5( # dense 추가에 따른 dense 4 -> dense 5 로 변경 240603
            self.dropout(
                nn.Tanh()(
                    self.dense4(v_T_) # 240603 dense 추가에 따른 dnese 3 -> dense 4 로 변경
                )
            )
        )
...
class GenClause(nn.Module):
    ...
    def __init__(self, config, clause_type="select"): # select max num : 6
        super().__init__()
        ...
        self.dense7 = nn.Linear(config.hidden_size * 4, config.hidden_size, bias=False) # dense 추가하거나 hidden size 변경하거나, ryansqlv2, ryansql' 를 만드는 것이 중요
        
        # gpt 에다가 코드 넣은 다음에 해보기 loss function을 변경해보기도 하기
        ...
        if clause_type != "groupby":
            ...
            elif clause_type == "where" or clause_type == "having": # utils_ko.py의 preds["where"] 과 매칭
...
class Model(nn.Module):
    def __init__(self, config, encoder=None):
        super().__init__()
        model_config = transformers.AutoConfig.from_pretrained(config.pretrained_model) # 모델 설정
        self.embedding = Embedding(config, encoder) # Embedding
        self.spc_embedding = nn.Embedding(len(Data.SPC), model_config.hidden_size) # SPC embedding
    
        self.q_self_attn = transformers.modeling_bert.BertSelfAttention(model_config) # Query 입력
        self.t_self_attn = transformers.modeling_bert.BertSelfAttention(model_config) # Table 입력
        self.d_self_attn = transformers.modeling_bert.BertSelfAttention(model_config) # Database 입력
        self.p_self_attn = transformers.modeling_bert.BertSelfAttention(model_config) # SPC 입력
        ...
        self.dense_s = nn.Linear(5 * config.hidden_size, config.hidden_size) # Dense
        ...
        self.gen_whe = GenClause(config, clause_type="where") ## 240515 이거 보기
    ...
    def gen_sql(self, V_Q, V_C, v_Q, v_T, v_D, q_mask, c_mask, d_mask, labels):
        ...
        # combine encodes
        v_S = self.dense_s(torch.cat([transform(v_Q, v_D), v_P], dim=-1)) # Dense
    ...
    def forward(self, x, mask, q_len, c_len, t_len, sql_mask, i, all_labels=None, table_map=None, val_1=None, val_2=None, val_3=None, train=True,
                tables=None, utt_ids=None, question_toks=None
                ): # 240516 val_1, val_2 추가 | 240518 i 추가 | 240521 val_3 추가 | 240617 question_toks 추가
        batch_size = x.size(0)
        index = i # 240518 추가
        ...
        for i in range(max_sql_num):
            ...
            if not train:
                ...
                if utt_ids: # utt_ids 는 dev.json 파일의 utterance_id 정보를 담고 있음
                    # 맨 처음 utt_ids : Whr_3005
                    """
                ...
                result, decode_sql_list = utils_ko.check_pred(
                    ...
                    , preds
                    , table_map[valid_idx]
                    , target_tables # prediction 결과에 맞는 target table의 정보를 담고 있음
                    , target_utt_ids # prediction 결과에 맞는 target utt_ids의 정보를 담고 있음
                    , question_toks # 240617 추가
                    , val_1 # 240516 추가
                    , val_2 # 240516 추가
                    , val_3 # 240521 추가
                    , index # 240518 추가
